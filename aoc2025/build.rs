...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (num, name) in &files {
        ...
        writeln!(bench_file, "use criterion::{{criterion_group, criterion_main, Criterion}};")?;
        writeln!(bench_file, "use aoc2025::Solution;")?;
        writeln!(bench_file, "use aoc2025::days::{}::Problem;", name)?;
        writeln!(bench_file)?;
        writeln!(bench_file, "const TEST_INPUT: &str = include_str!(\"../inputs/{:02}/test-input.txt\");", num)?;
        writeln!(bench_file, "const INPUT: &str = include_str!(\"../inputs/{:02}/input.txt\");", num)?;
        writeln!(bench_file)?;
        ...
        writeln!(bench_file, "    c.bench_function(\"{}_part1\", |b| b.iter(|| Problem.part_one(INPUT).unwrap()));", name)?;
        writeln!(bench_file, "}}")?;

        writeln!(bench_file, "fn bench_part2_real(c: &mut Criterion) {{")?;
        writeln!(bench_file, "    c.bench_function(\"{}_part2\", |b| b.iter(|| Problem.part_two(INPUT).unwrap()));", name)?;
        writeln!(bench_file, "}}")?;
